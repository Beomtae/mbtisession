...
function Result({ score, resetState }) {
  ...
  sessionStorage.setItem("mbti", result.mbti);
  ...
  const gifSrc = `${check}?${new Date().getTime()}`;

  return (
    <div className="content" id="result">
      {isLoading ? (
        <div className="loading-screen">
          <img src={gifSrc} id="img4"></img>
          <p>결과 생성중입니다...</p>
        </div>
      ) : (
        <>
          <img id="img" className="rounded-circle" src={result.img} />
